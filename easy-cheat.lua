...
local Window = redzlib:MakeWindow({
  Title = "Easy cheating ",
  SubTitle = "Just enjoy keyless",
  SaveFolder = "Easy cheating | Blox Fruits.lua"
})
...
if Sea3 then
  ...
  task.spawn(function()
    ...
    while task.wait() do
      if Map:FindFirstChild("KitsuneIsland") then
        KILabel:SetTitle("🏝️ Kitsune Island : Spawned | Distance : " .. Distance)
      else
        KILabel:SetTitle("🏝️ Kitsune Island : not Spawn")
      end
    end
  end)
  AutoSea:AddSection({"Sea"})
  AutoSea:AddToggle({Name = "⛴️ Auto Farm Sea",Callback = function(Value)
      ...
      task.spawn(function()
        while getgenv().AutoWoodPlanks do
          ...
          if VerifyToolTip("Sword") then
            RandomEquip = "🗡️ Sword"task.wait(2)
          end
